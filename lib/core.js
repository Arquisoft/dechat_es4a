...
const { SemanticGame, Loader } = require('semantic-chess');
const winston = require('winston');
const URI = require('uri-js');
const { format } = require('date-fns');
const rdfjsSourceFromUrl = require('./rdfjssourcefactory').fromUrl;
class Message {
  Message(content, author, ofChat) {
    this.Created = new Date();
    this.Created = this.Created.getDate() + "/"
...
class SolidChessCore {
   ...
   * @returns {SemanticChat}: the newly created chess game
   */
  async setUpNewChat(userDataUrl, UserWebId, FriendWebId, n) {
